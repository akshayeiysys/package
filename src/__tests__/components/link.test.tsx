...
it("renders link correctly", () => {
  const { asFragment } = renderWithTheme(<Link href="http://3.143.197.157">Link</Link>);
...
it("renders link external link correctly", () => {
  const { asFragment } = renderWithTheme(<LinkExternal href="http://3.143.197.157">Link</LinkExternal>);
  expect(asFragment()).toMatchInlineSnapshot(`
    <DocumentFragment>
      .c0 {
      color: #1FC7D4;
